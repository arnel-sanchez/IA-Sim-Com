...
from errors import UnknownCharacterError, UnexpectedCharacterError, UnbalancedBracketsError
...
class Tokenizer:
    def __init__(self):
        ...
        self.operators = dict([("=", TokenType.T_ASSIGN), ("==", TokenType.T_EQUAL_OP),
    ...
    def tokenize(self, file: str, input_text: str) -> [TokenType]:
        ...
        while self.f_pointer < len(self.input_text) and self.error is None:
            ...
            if current == ' ' or current == '\t':
                ...
                opt = self.operators.get(current)
                if opt:
                    self.operator(current, opt)
                else:
    ...
    def operator(self, token: str, opt: TokenType):
        while self.f_pointer < len(self.input_text):
            ...
            opt = new_opt
            self.f_pointer += 1
        self.tokens.append(Token(opt, self.line, self.column))
        self.f_pointer -= 1
